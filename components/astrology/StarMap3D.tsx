...
export default function StarMap3D({ birthDate, birthTime, birthPlace, onMapGenerated }: StarMap3DProps) {
  ...
  const calculatePlanetPosition = (planetIndex: number, date: Date, hour: number, minute: number) => {
    ...
    const planetAngle = baseAngle / orbitalPeriods[planetIndex] + timeOfDay * 0.1;

    const distances = [180, 140, 160, 200, 240, 300, 360, 420, 480];
    const radius = distances[planetIndex];

    const inclination = Math.sin(planetAngle) * 0.2;

    return {
      x: Math.cos(planetAngle) * radius,
      y: Math.sin(inclination) * 40,
      z: Math.sin(planetAngle) * radius,
      degree: Math.floor((planetAngle * 180 / Math.PI) % 360)
    };
  };
  ...
  }, [birthDate, birthTime, birthPlace, mounted]);
